...
const Cards = () => {
  ...
  const { cards, itemsPerPage, pagesAmount, isInitialRequestDone, isLoading } = useSelector((s) => s.poke);
  ...
  return (
    <main className={pagesAmount === +currentPage ? 'main-section align-items' : 'main-section'}>
      {cards
        .filter((it, index) => index >= (+currentPage - 1) * itemsPerPage && index <= +currentPage * itemsPerPage - 1)
        .map((it) => {
          return (
            !isLoading && (
      ...
      {isInitialRequestDone && !cards.length && <p id='error'>Неверный URL, либо нет таких покемонычей!</p>}
    </main>
  );
};
